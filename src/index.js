...
async function createApp() {
  ...
  try {
    result = await prompts(
      [
        ...
        {
          type: () =>
            (0, packageName_1.isValidPackageName)(targetDir) ? null : "text",
          name: "packageName",
          message: reset("Package name:"),
          initial: () => (0, packageName_1.toValidPackageName)(targetDir),
          validate: (dir) =>
            (0, packageName_1.isValidPackageName)(dir) ||
            "Invalid package.json name",
        },
        {
          type:
            template && templates_1.TEMPLATES.includes(template)
              ? null
              : "select",
          name: "framework",
          message:
            typeof template === "string" &&
            !templates_1.TEMPLATES.includes(template)
              ? reset(
  ...
  const pkgInfo = (0, pkg_1.pkgFromUserAgent)(
